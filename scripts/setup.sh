...
# ============ Release 模式: symlink 到 ~/.local/bin/kallax ============
# EPIC-128: 让 `bash setup.sh --release vX.Y.Z` 一键装完跟源码 install 一样可直接用 `kallax` 命令
if [[ "$SOURCE_KIND" == "release" && -n "$RELEASE_TAG" ]]; then
  KALLAX_BIN_DST="${HOME}/.local/bin/kallax"
  # Node dist 路径
  NODE_ENTRY="$KALLAX_ROOT/node/dist/index.js"
  if [ ! -f "$NODE_ENTRY" ]; then
    NODE_ENTRY="$(find "$KALLAX_ROOT/node/dist" -name 'index.js' | head -1)"
  fi

  if [ -f "$NODE_ENTRY" ] && command -v node >/dev/null 2>&1; then
    mkdir -p "$(dirname "$KALLAX_BIN_DST")"
    cat > "$KALLAX_BIN_DST" <<WRAPPER
#!/usr/bin/env bash
# KALLAX CLI wrapper — generated by scripts/setup.sh (release mode)
# tag=${RELEASE_TAG} commit=${GITHUB_SHA:-local}
exec node "${NODE_ENTRY}" "\$@"
WRAPPER
    chmod +x "$KALLAX_BIN_DST"
    echo "✅ 已链接 CLI: ${KALLAX_BIN_DST} → node ${NODE_ENTRY}"
  else
    echo "⚠️  未找到 node entry ($NODE_ENTRY) 或 node 不在 PATH, 跳过 symlink"
  fi

  # Rust engine 安装 (如果 archive 里有)
  RUST_BIN_SRC="$KALLAX_ROOT/rust/kallax-engine"
  RUST_BIN_DST="${HOME}/.local/bin/kallax-engine"
  if [ -f "$RUST_BIN_SRC" ]; then
    mkdir -p "$(dirname "$RUST_BIN_DST")"
    cp "$RUST_BIN_SRC" "$RUST_BIN_DST"
    chmod +x "$RUST_BIN_DST"
    echo "✅ 已部署 Rust engine: ${RUST_BIN_DST}"
  fi
