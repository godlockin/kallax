#!/usr/bin/env bash
# scripts/verify/level-3.sh — L3 接线正确 (4-expert 评审) (武器 2 Iter 5)
#
...
#   3. 全部 PASS = exit 0, 任何 FAIL = exit 1
#
# Exit codes:
#   0  PASS — 4 expert 全 PASS (或 --dry-run)
#   1  FAIL — 任一 expert FAIL / review 缺失
#   2  ERROR — 参数错
...
for expert in "${EXPERT_LIST[@]}"; do
    ...
    if [ ! -f "$REVIEW_FILE" ]; then
        fail "expert=$expert review missing: $REVIEW_FILE"
        echo ""
        continue
    fi
...
" 2>&1)
    ...
    if [ "$RC" -ne 0 ] || [ "$EXPERT_STATUS" = "parse_error" ]; then
        fail "expert=$expert review JSON parse error"
        echo ""
        continue
    fi
...
# Summary
echo "=========================================="
echo "L3 Summary: $PASS_COUNT PASS, $FAIL_COUNT FAIL (of $TOTAL)"
echo "=========================================="
