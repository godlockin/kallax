...
cmd_next() {
    ...
    if [ -n "$eligible_ticket" ]; then
        # Check if input is a priority label (P0/P1/P2/BLOCKED) or ticket_id
        local priority priority_num
        if [[ "$eligible_ticket" =~ ^(P0|P1|P2|BLOCKED)$ ]]; then
            # Priority label passed directly - use it
            case "$eligible_ticket" in
                P0) priority_num=0 ;;
                P1) priority_num=1 ;;
                P2) priority_num=2 ;;
                BLOCKED) priority_num=3 ;;
            esac
            priority="$eligible_ticket"
        else
            # Treat as ticket_id
