...
tickets_served: []
trigger: API,接口慢,数据库,SQL,查询慢,索引,n+1,事务,缓存,性能,后端,服务端,数据层,连接池,锁竞争,慢查询,接口慢怎么优化,数据库索引怎么加,请求超时,请求超时了,5xx,500,连接池满,死锁,慢SQL,N+1查询,后端报错,服务端崩,数据库连接超时,接口怎么优化,缓存击穿
output_format: |
  ## 亮点
  - 接口设计清晰,符合REST语义
  - 错误处理完善,panic禁用于生产
