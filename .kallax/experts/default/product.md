...
git log --oneline -1
git show HEAD:.kallax/experts/default/product.md >/dev/null && echo "L1 PASS: file exists + git anchor traceable" || echo "L1 FAIL"
```

### L2 test stdout (实质性)
```bash
# 真实 raw stdout, 不接受 "should work" / "looks correct" / silent
# product persona: state.json + tickets_served 验证 + 真实 raw output
test -f .kallax/experts/default/product.md && wc -l .kallax/experts/default/product.md && echo "L2 PASS: raw stdout captured (no 'should work' / silent)" || echo "L2 FAIL"
```
...
bash scripts/verify/check-fact-forcing-preflight.sh .kallax/experts/default/product.md 2>&1 | tail -20 && echo "L4 PASS: independent witness verified" || echo "L4 FAIL"
```