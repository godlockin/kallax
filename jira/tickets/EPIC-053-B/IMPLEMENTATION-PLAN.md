...
实现 4-Level 证据链强制约束 — Performer 报 PASS 必须含全部 4 层证据, 缺 1 算 FAIL. 12 KPI falsification 反复 (EPIC-024/028/031/036/037/039-B) 系统级治根.
...
| AC2 | 6 case 全 PASS (4-Level 完整 + 缺 1 Level FAIL + 假 git-anchor FAIL + 假 test stdout FAIL + 5 扩展组不全 FAIL + 缺独立见证签名 FAIL) | 跑 `tests/integration/kpi-evidence-chain-test.sh` |
| AC3 | `node/src/core/output-verifier.ts` 升级 — Performer 报 PASS 必须含 4-Level 证据, 缺 1 算 FAIL | TypeScript 编译 + 测试 |
| AC4 | 12 KPI falsification 反复 治根闭环 | 4-Level evidence chain 全覆盖 PASS 路径 |
...
`node/src/core/output-verifier.ts` 当前用 4-Level Fact-Forcing (L1 git / L2 substance / L3 wiring / L4 data flow), 但**没有强制 Performer 报 PASS 时必须含 4-Level 证据**. 本 ticket 新增 `verifyPassEvidence()` 方法, 接受 4-Level 证据结构, 缺一算 FAIL.
...
**创建**:
- `scripts/verify/kpi-evidence-chain.sh` — 4-Level 证据链核心实现
...
- `scripts/audit/independent-witness.sh` — 集成 4-Level 证据链接口 (Rule 30/31 联合)
- `node/src/core/output-verifier.ts` — 新增 `verifyPassEvidence()` 方法, 强制 4-Level 证据
...
| Case | 场景 | 期望 |
|------|------|------|
| 1 | 4-Level 完整 + 全部 PASS | OK (exit 0) |
...
| 风险 | 缓解 |
|------|------|
| 4-Level 证据链定义不清晰 | 显式 truth table (4 维度都要, 缺 1 算 FAIL) |
...
| 简化 4-Level 证据链 | 严格 4-Level, 缺 1 算 FAIL, 不允许降级 |
| 复制 EPIC-053-A 模式但不真实现 | 4-Level 各自独立验证逻辑, 不复用 EPIC-053-A 实现 |