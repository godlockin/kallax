...
| 3. testing → main | `gh pr create --base main` (FF) | full e2e + decision matrix 25 cells | **T1/T2/T3 分级 + comment 验证** | 防止 v3.8.0 25 测试全过的假 PASS |
...
5. **conflict check**: PR 必先 `git fetch origin <base>` + `git diff --check` 验 0 conflict
6. **smoke retention**: PR 必跑 `bash scripts/check-smoke-retention.sh` (阈值来自 EPIC-174)
7. **PR-2 v2 修正**: testing → main 在 FF 关系下独立 PR 不可行, 走 FF push + comment 验证
...
- **写段禁 jargon (EPIC-252 纠正)**: merge commit 的 staged diff 把新写段算新增行, 含黑名单词会被 `check-decorative-claim.sh` 拦. 写段前查 `jira/tickets/.jargon-blacklist.json` 直接避开, 不用 bypass
