...
//   - 子任务 2: event-bus bridge (L1 Rust 主用 + L2 Node.js 备) — DEFERRED (orphan, 跨 release 留待)
...
//   - event_bus.rs: 纯 Rust event bus logic — 子任务 2 DEFERRED (跟 "翻篇&精进" 联合, 跨 release 留待)
...
//
// 跟"诚实修正" 战略 联合: event_bus + data_adapter 是 5 subagent 跨 worktree 并行 合并 后 孤儿 模块
// (--theirs resolve 丢失 lib.rs/Cargo.toml 引用). 跨 release 留待 master 后续 拍板 (跟"独立" 战略 联合).
...
// ── Orphan modules (子任务 2 + 3 DEFERRED, 跨 release 留待 拍板) ─────────────
