    ...
    match action {
        TaskAction::Create {
            ...
            output_result(
                format,
                "ticket_created",
                serde_json::json!({ "ticket_id": id.as_str() }),
            );
        }
        TaskAction::Claim {
            ticket_id,
        ...
        TaskAction::List { status } => {
            ...
            output_result(
                format,
                "tickets",
                serde_json::json!({ "tickets": ticket_list }),
            );
        }
    ...
    match action {
        ...
        ConductorAction::Heartbeat { conductor_id } => {
            output_result(
                format,
                "heartbeat_ok",
                serde_json::json!({ "conductor_id": conductor_id }),
            );
        }
    }
    Ok(())
    ...
    match action {
        VerifyAction::Run { ticket_id } => {
            output_result(
                format,
                "verify_run",
                serde_json::json!({ "ticket_id": ticket_id, "result": "pending" }),
            );
        }
        VerifyAction::Kpi { ticket_id } => {
            output_result(
                format,
                "verify_kpi",
                serde_json::json!({ "ticket_id": ticket_id, "kpi": null }),
            );
        }
    }
    Ok(())
