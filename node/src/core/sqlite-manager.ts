...
function createAsyncWrapper(_db: Database.Database, dbPath: string): SQLiteManager['async'] {
  ...
  return {
    createTicket: async (t) => ok(await call<void>('createTicket', t)),
    getTicket: async (id) => ok(await call('getTicket', id)),
    updateTicket: async (id, u) => ok(await call<void>('updateTicket', id, u)),
    listTickets: async (f) => ok(await call('listTickets', f)),
    createTask: async (t) => ok(await call<void>('createTask', t)),
    getTask: async (id) => ok(await call('getTask', id)),
    updateTask: async (id, u) => ok(await call<void>('updateTask', id, u)),
    listTasks: async (f) => ok(await call('listTasks', f)),
    claimTask: async (tid, pid) => ok(await call<boolean>('claimTask', tid, pid)),
    registerInstance: async (i) => ok(await call<void>('registerInstance', i)),
    getInstance: async (id) => ok(await call('getInstance', id)),
    updateInstance: async (id, u) => ok(await call<void>('updateInstance', id, u)),
    listInstances: async (f) => ok(await call('listInstances', f)),
    updateHeartbeat: async (id) => ok(await call<void>('updateHeartbeat', id)),
    getStaleInstances: async (t) => ok(await call('getStaleInstances', t)),
    enqueueMessage: async (m) => ok(await call<void>('enqueueMessage', m)),
    dequeueMessage: async (t) => ok(await call('dequeueMessage', t)),
    peekMessages: async (l) => ok(await call('peekMessages', l)),
    getStats: async () => await call<DatabaseStats>('getStats'),
  };
}
