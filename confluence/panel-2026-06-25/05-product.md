...
**F1: Phase 1 EPIC 计数 错** — Phase 1 报告 claim "22 EPICs + 14 archive = 36 epic.json", 实际 `jira/epics/` 31 EPIC dirs + `jira/epics/_archived/` 6 empty = **37 epic.json 实际存在** (file:line `ls jira/epics/` = 31, `jira/epics/_archived/` 6, total 37, vs Phase 1 claim 36). Phase 1 计数 从根源修复 需 re-verify.
...
**R1: 文档计数 不可信 风险** — Phase 1 claim "22 EPICs" 实际 31, 治理 panel 后续 决策 0 实际 baseline. 后续 7 重复类型 / 7 archive 路径 从根源修复 全部 依赖 此 baseline. 从根源修复 = 计数 重新 re-verify (跟 Phase 1 9 顶层 README 缺 联合 从根源修复).

**R2: paper-active 反讽 风险** — 14 EPIC 标 active 但 0 in_progress ticket, 跟 "独立" 战略 联合 0 ai-auto 决策 冲突 (Conductor 0 跟 master 拍 active → done 转换). 风险 = Conductor 心跳 5 问 Q2 (Slaver 状态) 看到 active 但 0 work, 0 实际 blocking signal. (跟 EPIC-039 Sprint 4 ticket 状态自动同步 联动 从根源修复)

**R3: EPIC-058 ticket 缺失 路径 风险** — 5 tickets in epic.json 全部 done, 但 0 ticket.json 从根源修复 "完成 无 证据" 反讽 模式 (跟 v2.4.0 0 实际变化 假动作 联合). Risk = EPIC-058 5 票 done 不可 audit (0 PASS 报告 + 0 test output + 0 commit ref in ticket.json), 跟 EPIC-059-D Fact-Forcing 联合 0 校验. (跟 c091d92 commit 撤回 模式 区别, 0 ticket evidence)
...
**R5: ticket.json secondary status 从根源修复 风险** — v2.7.4 D5 schema 加 pending/deferred/failed 3 secondary status, 跟 v1.0 8 状态机 互补 (file:line `jira/schemas/ticket-schema.md:73-77`). 8 pending + 4 blocked + 1 failed = **13 ticket (10%) 走 secondary 状态**, 跟 "0 隐藏 debt" 战略 联合 需 Phase 2 9 专家 共识 决策. (跟 v2.7.4 BE-18 "留 3 项 永久 debt" 模式 联合)
...
**P2: epic_index.json 跨 release 留待 从根源修复 (跟 EPIC-054-C epic-state-machine.md:130 TODO 联合)** — 22 EPIC 缺失 + 1 status drift (EPIC-058), 跟 epic-state-machine.md §6 "未来加 epic index sync 子命令" 留待 联合, **不** 在 Phase 2 9 专家 强制 修复 (跟"独立" 战略 联合 master explicit 拍, 0 ai-auto).
...
- **0 跨 session 拍板** 7 重复 类型 从根源修复 (跟 Phase 1 §1.4 留待 联合)
