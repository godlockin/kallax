...
|---|---------|----------------------|
| **F1** | **Level 编号 反向 bug** — 同一字面 "Level" / "L" 在不同 doc 含义相反, 从根源修复 reader 错位 | `docs/architecture/DEGRADATION-STRATEGY.md:31-90` L3=Full Production (top), L0=Shell (bottom); `docs/architecture/FRAMEWORK.md:116-149` L3=Rust (top), L1=Shell (bottom); `confluence/decisions/EPIC-060-A-ROADMAP-2026-06-19.md:66-78` L0=Shell (bottom), L3=Web (top). 3 套 L 编号 × 2 套语义 |
...
| **F6** | **EPIC-060 17 个 decision doc 0 反映在 docs/architecture/** — 分布式路线图 双轨 平行 | `confluence/decisions/EPIC-060-A-ROADMAP-2026-06-19.md` (5 phases 92h P0-P2) + `EPIC-060-B-RUST-INVEST-2026-06-19.md` (3 phases 52h) + `EPIC-060-C-LAYERS-2026-06-19.md` (4→5 层 4h) 17 files 落地, 但 `docs/architecture/AGENT-PROTOCOL.md`, `DAG-SCHEDULER.md`, `ISOLATION-STRATEGY.md`, `ELECTION-SYSTEM.md` 0 任何 EPIC-060 交叉引用, 从根源修复 P3 留待 跟架构 文档 脱钩 |
...
| **R2** | **API 文档 信任 风险** (F3, F4) | docs/api/ 共 5 端点 跟实际 路径/method 错位, 0 从根源修复 front-end / Performer / curl 调用 100% 失准 | 0 自动 校验 script; 跨 release 留待 8-15 处 endpoint rename, R3 (Phase 1) 已提 "链接 断 风险", 从根源修复需 API 同步 校验 |
| **R3** | **数据模型 ID 错位** (F5) | jira/tickets/ 实际 EPIC-016-A/EPIC-060-A 模式, 跟 TASK-NNN schema 跟 TICKET-XXX API doc 三方 错位, 0 从根源修复 epic→ticket 1:N 关系 文档 失真 | ticket-schema.md:8 跟实际 0 reconcile; Phase 1 R3 "从根源修复 反复 风险" 已 命中 |
| **R4** | **EPIC-060 留待 跨 release 留待 风险** (F6) | P3 留待 doc 0 反映在架构 概览, 从根源修复 "看架构 0 知有分布式 路线图", 跟 v2.0.7 PHASE-014 模式 一致 0 入口 沉淀 | 0 缓解; 推荐 ROADMAP.md + FRAMEWORK.md 显式 link EPIC-060-A/-B/-C 决策 doc (从根源修复 P3 入口 沉淀) |
| **R5** | **DEGRADATION-STRATEGY 描述 跟 Rust 实际 状态 失真** (F8) | docs/architecture/DEGRADATION-STRATEGY.md 描述 Rust 8ms 启动 + 12MB 内存, 跟 "5 crates 0 投入" 现状 错位, 从根源修复 Phase 0 实际 未达 docs/ 描述 | 0 缓解; DEGRADATION-STRATEGY.md 需显式 "Rust 实施 状态 留待 EPIC-060-B" 标注 (跟"诚实修正" 战略 联合) |
| **R6** | **R2 链接 断 风险 (Phase 1 R2) 实证 升级** | Phase 1 R2 "跨 release 大量 rename 风险" 预测 命中 — F1-F8 8 个 finding 全部 是 "文档 跟 实际 错位", 0 是 "rename 链接 断", 从根源修复 范围 升级 0 仅为 rename | 0 缓解; 跨 release 留待 "API + 路径 校验 script" (跟 R2 联合) |
...
| # | 主题 | 推荐 | 范围 |
|---|------|------|------|
| **K1** | **架构 doc 跟 实际 代码 reconcile (0 强制 拍板 0 增 Rule)** | 跨 release 留待 master explicit 拍 1 共识: docs/architecture/ 0 强 rename, 0 强 "L 编号 统一", 仅 "epic 从根源修复" — 0 拍 1 命名, 留待 主公后续 | 跟"独立" 战略 联合 |
| **K2** | **F1 L 编号 反向 bug — 从根源修复 1 表 reconcile** | 跨 release 留待 从根源修复 1 表: "L 编号 → 实际 层" (EPIC-060-A L0=Shell, L1=Rust, L2=Node.js, L3=Web) 写进 `docs/architecture/ROADMAP.md` 头部, 0 重写 DEGRADATION-STRATEGY/FRAMEWORK 文本 | 跟"诚实修正" 战略 联合 |
...
| **K5** | **F5 Ticket ID 3 套 错位 — 1 schema 从根源修复** | 跨 release 留待: `jira/schemas/ticket-schema.md:8` 跟 `jira/tickets/` 实际 目录 对齐 — 0 强拍 1 命名, 留待 跨 release 重新 从根源修复 | 跟"翻篇&精进" 战略 联合 0 增 Rule |
...
- **0 跨 release 留待 统一 命名** — F5 3 套 Ticket ID 跨 release 留待 从根源修复, 跟"翻篇&精进" 战略 联合 0 强拍
- **0 跟踪 inbox/human_feedback.md 写 K2-K6 决定** — 留待 主公 后续 拍板 时 由 Conductor 写入 (跟 PROCESS.md:25-26 联合)
- **0 跨 release 留待 8 finding 一次性 从根源修复** — F1-F8 跨 release 累计 文档化 实际 错位, 0 拍 1 batch 从根源修复, 留待 跨 release 重新 从根源修复 (跟 BE-14 联合)
...
> **DEPTH medium**: 跨 release 累计 文档化 实际 可执行 路径, 0 抽象 从根源修复
