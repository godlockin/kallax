...
**日期**: 2026-06-29
**Reviewer**: Architect (Performer/reviewer sub-role, 跟 V310-A / V350-A §4 architect angle 1:1 联合)
**范围**: 架构 / 边界 / 选型 / 微服务 / 模块
**Base**: miao 1b9694b (v3.5.0-hotfix1 实战 + LESSONS-LEARNED 5 release 累计)
**方法**: Forward review 找强项 + 跨项目 1:1 对比 + 5 release 累计 借鉴 Gap

> **不重复 V310-A / V350-A**: V310-A §4 (5 levels / 3 层降级 / 1 binary / 6 武器 / per-session parity / CLAUDE.md 3.3KB) 已覆盖 KALLAX 自身架构; 本文件 1:1 联合 V310-A §4 但 pivot 到 **eket vs KALLAX 跨项目** 视角, 重点是 **5 release 累计 借鉴 实战** (eket ioredis + graceful-exit 1 次) + **关键 Gap**.
...
| **Shell 兜底** | `scripts/graceful-exit.sh` 166 行 (跟 eket Level 4 1:1 联合) | `lib/adapters/hybrid-adapter.sh` | **1:1** (但 KALLAX v3.5.0 实战 1 次, evidence `docs/evidence/v3.5.0/graceful-exit-actual.txt` 5 行 落地) |
...
**Forward 强项 (KALLAX 胜)**:
- 5 crates 1 binary 整合 (跟 eket 4 crates 1 binary 同源, 1:1 整合哲学 1:1 借鉴)
...
| **Misc (4)** | `kallax mode:set + role:switch + worktree:create + skill:list` | `eket roadmap:update + spike:{create,complete} + skill:extract + version` | **KALLAX 独有 mode + role + worktree** (跟 Q18 3 模式 + Rule 15 sub-role + Rule 14 worktree 1:1 联合) |
...
| **降级 落地** | L3 Shell `scripts/graceful-exit.sh` 166 行 (跟 eket Level 4 1:1 联合, v3.5.0 实战 1 次 evidence) | L0 Shell `lib/adapters/hybrid-adapter.sh` (跟 eket 1:1 落地) | **1:1 借** (KALLAX 实战 1 次, eket 早落地) |
...
**Forward 强项 (KALLAX 胜)**:
- 5 levels 实做 (5 独立脚本 + CLI wrapper) 跟 eket 5 levels 名字 only 区分, 跟 V310-A §2.2 5 levels scripts 互不耦合 1:1 验证
...
- 4 roles 区分 (1+4 容量) 跟 eket 1+1 区分, 跟 Q15 决策 联合, 跟 V310-A §4.7 "KALLAX = 3 层 (master → Conductor/Performer → sub-role) + 4 sub-roles 并行" 1:1 验证
...
- KALLAX Node.js 35 modules 比 eket 12 modules 多 23, 跟 eket 精简哲学偏离 (但 KALLAX 实战覆盖更广, 跟 V310-A §4.6 "per-session parity 0.92x" 1:1 验证 lazy load 设计 1:1 working as designed)
...
| **L3 Shell 兜底** | ✅ `scripts/graceful-exit.sh` 166 行 (跟 eket Level 4 1:1 联合, v3.5.0 实战 1 次 evidence) | ✅ `lib/adapters/hybrid-adapter.sh` 跟 L0 复用 | **1:1 借** (但 KALLAX v3.5.0 实战 1 次, eket 早落地) |
| **降级日志** | ✅ `logger.warn({event: 'degradation_triggered', from, to, reason})` (跟 ARCHITECTURE.md §9 1:1 联合) | ❌ 无显式降级日志 (跟 eket 1:1 借命名) | **KALLAX 胜** (显式降级日志 实做) |
...
**Forward 强项 (KALLAX 胜)**:
- 显式降级日志 (`logger.warn({event: 'degradation_triggered', from, to, reason})`) 跟 eket "无显式降级日志" 区分, 跟 ARCHITECTURE.md §9 1:1 联合
- L1/L2 触发条件更明确 (timeout + crash × N retry), 跟 eket `waitForRustServer() 3s poll` 区分
- v3.5.0 graceful-exit.sh 166 行 跟 eket Level 4 1:1 联合 + 实战 1 次 evidence (`docs/evidence/v3.5.0/graceful-exit-actual.txt` 5 行)
...
- graceful-exit 跟 eket Level 4 优雅退出 1:1 命名 (跟 v3.4.0 graceful-exit.sh 跟 eket Level 4 1:1 联合)
...
| **L4 独立见证** | ✅ `scripts/verify/level-4.sh` 3.6K + `kallax witness:spawn TICKET --independent` (跟 Q18 L4 "主公拍" cell 1:1 联合) | ❌ 名字 only | **KALLAX 胜** (独立 subagent 重跑 L1-L3, 治根 "瞒报" 反模式) |
...
| **集成测试 5 levels** | ✅ `tests/integration/5-levels-test.sh` PASS (跟 V310-A §3.1 "23/23 4-Level cells PASS" 1:1 验证, v3.5.0 5 levels 集成测试 维持) | ❌ 5 levels 集成测试 0 | **KALLAX 胜** |

**核心 Gap (5 release 累计 借鉴 实战)**:

KALLAX 5 levels 实做 跟 eket 5 levels 名字 only 区分, **5 release 累计 0 退步**, 跟 V350-A §5.1 "5 levels 独立" 5/5 维度 PASS 联合. KALLAX 治根 了 eket 哪些 痛点:

| eket 5 levels 痛点 | KALLAX 治根 | 5 release 累计 |
|-------------------|------------|----------------|
...
| L4 "独立见证" 概念 | `kallax witness:spawn TICKET --independent` + 新 session 重跑 L1-L3 + 不可被原 subagent 篡改 (跟 Q18 L4 "主公拍" cell 1:1 联合) | v3.1.0 落地 → v3.5.0 hotfix 16 P-001 (Iter 1 自打脸 治根) + P-002 ("0 装饰引用" self-contradict 治根) 全跑 independent-witness 验证, 跟 V350-A 5 维度 PASS 联合 |
...
| 集成测试 5 levels 0 | `tests/integration/5-levels-test.sh` PASS + `tests/integration/l3-dry-run-test.sh` 4/4 PASS | v3.1.0 → v3.5.0 5 release 累计 0 退步, 跟 V310-A §3.1 "23/23 4-Level cells PASS" 1:1 验证 |
...
|--------|---------------------------|----------|----------|
| **ioredis parity check** | `docs/evidence/v3.5.0/ioredis-parity-check.md` 38 行 (跟 "诚实修正" 联合 "实际 跑过 诚实"), `node/package.json:ioredis ^5.4.0` 验证, 跟 eket 分布式锁 (SETNX) + 分布式队列 (Pub/Sub) 1:1 验证, 跟 v3.0.0 master-election.ts 三级选举 (Redis SETNX + SQLite + File) 1:1 验证 | eket `fred` crate (async) + ioredis 实战 (TASK-141 SSE 5 态事件流补完 P0 Sprint1) | **1:1 借鉴** (KALLAX v3.5.0 实战 1 次, eket 早落地; 跟 "诚实修正" 战略 1:1 联合) |
| **graceful-exit.sh 实战 1 次** | `docs/evidence/v3.5.0/graceful-exit-actual.txt` 5 行 + `docs/evidence/v3.5.0/graceful-exit-dryrun.txt` 5 行 + `scripts/graceful-exit.sh` 166 行 (跟 eket Level 4 优雅退出 1:1 联合, 6 步 落地: audit chain + hook server + web dashboard + Node.js + Rust binary + Shell 兜底) | eket Level 4 优雅退出 (跟 eket architecture.md "Rust 内部降级链" 1:1) | **1:1 借鉴** (KALLAX v3.5.0 实战 1 次, eket 早落地; 跟 V310-A §4.5 "per-session parity 0.92x" 1:1 联合) |
| **master-election.ts 三级选举** | `node/src/core/master-election.ts` 370 行 (跟 v3.0.0 跟 eket 1:1 借, 三级选举 Redis SETNX + SQLite + File) | eket 三级 Master 选举 (Redis SETNX + SQLite + File) | **1:1 借** (跟 eket master-election 1:1 借鉴, 跟 eket architecture.md §"Rust 内部降级链 (election.rs)" 1:1 联合) |
| **HEARTBEAT + SSE** | `node/src/core/heartbeat-monitor.ts` + `sse-bus.ts` (跟 eket SSE 5 态事件流 1:1 借鉴) | eket TASK-141 SSE 5 态事件流补完 (P0 Sprint1) | **1:1 借** (KALLAX 已落地, eket TASK-141 P0 Sprint1 计划) |
| **circuit-breaker** | `node/src/core/circuit-breaker.ts` (跟 eket circuit_breaker.rs 1:1 借, closed/open/half_open 3 态) | eket `circuit_breaker.rs` (closed/open/half_open, 退避重试) | **1:1 借** (跟 eket 1:1 借鉴) |
| **cache** | `node/src/core/...` (LRU + TTL, 跟 Rule 4 资源管理规范化 联合) | eket `cache.rs` L1 moka (300s) + L2 Redis 二级缓存 | **eket 胜** (eket 多 1 级 L2 Redis 缓存) |
| **knowledge FTS5** | `node/src/core/...` (跟 eket `knowledge.rs` SQLite FTS5 BM25 1:1 借) | eket `knowledge.rs` SQLite FTS5 (BM25 评分) | **1:1 借** (跟 eket 1:1 借鉴) |
| **recommender TF-IDF** | `node/src/core/...` (跟 eket `recommender.rs` TF-IDF 余弦相似度 1:1 借) | eket `recommender.rs` TF-IDF 余弦相似度推荐 (CJK unigram tokenize) | **1:1 借** (跟 eket 1:1 借鉴) |
| **conflict-resolver** | `node/src/core/...` (跟 eket `conflict_resolver.rs` first_claim_wins / lock_queue / priority 1:1 借) | eket `conflict_resolver.rs` (first_claim_wins / lock_queue / priority) | **1:1 借** (跟 eket 1:1 借鉴) |
| **lock** | `node/src/core/...` (跟 eket `lock.rs` Redis SETNX + 内存 fallback + FIFO 等待队列 1:1 借) | eket `lock.rs` Redis SETNX + 内存 fallback + FIFO 等待队列 | **1:1 借** (跟 eket 1:1 借鉴) |
| **saga 5 步** | `node/src/core/saga-executor.ts` (跟 eket `saga.rs` Saga 补偿事务 forward/compensate 1:1 借) | eket `saga.rs` Saga 5 步 (ValidateTicket → CommitWork → UpdateStatus → NotifyMaster → Record) | **1:1 借** (跟 eket 1:1 借鉴) |
| **dag 解析** | `node/src/core/dag-generator.ts` + `dag-visualizer.ts` (跟 eket `dag.rs` Kahn 拓扑 + 关键路径 + 循环检测 1:1 借) | eket `dag.rs` DAG 解析、拓扑排序 (Kahn)、关键路径、循环检测 | **1:1 借** (跟 eket 1:1 借鉴) |
...
**Forward 强项 (KALLAX 胜)**:
- v3.5.0 实战 1 次 (commit `096eafe` line 22 "实战 eket ioredis 1 次 ... 实战 graceful-exit 1 次") 跟 "诚实修正" 战略 1:1 联合
- 5 release 累计 0 跳 release (v2.7.5 → v2.7.6 → v3.0.0 → v3.1.0 → v3.2.0 → v3.3.0 → v3.4.0 → v3.5.0 演化路径 1:1 验证, 跟 "翻篇&精进" 战略 联合)
- 0 跳 release + 实战 1 次 联合 治根 "0 实际变化 假动作" (跟 V310-A §2 强项 联合)
...
**1:1 对齐**:
- master-election / circuit-breaker / knowledge FTS5 / recommender TF-IDF / conflict-resolver / lock / saga / dag 8 个模块 1:1 借 (跟 eket 1:1 借鉴)
- graceful-exit 跟 eket Level 4 1:1 命名 (v3.5.0 实战 1 次, 跟 eket 1:1 联合)
- ioredis parity check 跟 eket 1:1 命名 (v3.5.0 实战 1 次, 跟 eket 1:1 联合)

---

## 6. 关键 Gap (KALLAX v3.6.0 应 治根 的 架构 Gap)

| Gap # | 描述 | 治根 路径 | 关联 Rule |
|-------|------|----------|----------|
| **Gap 1** | KALLAX L0 Shell 落地 跟 eket L0 差距 (eket L0 是 "100% 可用基底 ⭐⭐⭐⭐⭐", KALLAX L3 Shell 兜底 跟 eket L0/L3 联合 但 缺 eket L0 单独落地) | v3.6.0 候选: 落地 L0 Shell (跟 eket `lib/adapters/hybrid-adapter.sh` 1:1 借鉴, 跟 eket architecture.md "Level 0: Shell" 1:1 联合) | Rule 14 (worktree) + Rule 1 (并行隔离) |
| **Gap 2** | KALLAX Node.js 35 modules 比 eket 12 modules 多 23, 跟 eket 精简哲学偏离 (虽然实战覆盖广) | v3.6.0 候选: 评估 哪些 modules 是 "dead_code" 删, 跟 eket 12 modules 1:1 对齐精简 (跟 V310-A §2.1 "27 warnings 集中 parsers.rs dead_code" 联合) | Rule 5/8 (Rule of 500) + Rule 12 (3 模式) |
| **Gap 3** | eket TASK-141 SSE 5 态事件流补完 (P0 Sprint1), KALLAX v3.5.0 还没到 SSE 5 态 完整覆盖 (`node/src/core/sse-bus.ts` 已存在 但 5 态 完整补完 待落地) | v3.6.0 候选: SSE 5 态 完整补完 (跟 eket TASK-141 P0 Sprint1 1:1 借鉴) | Rule 18 (KPI falsification) + Q18 L4 (主公拍) |
| **Gap 4** | eket `cache.rs` L1 moka + L2 Redis 二级缓存 比 KALLAX LRU + TTL 多 1 级 L2 Redis, KALLAX 缺 L2 Redis 二级缓存 | v3.6.0 候选: 跟 eket 1:1 落地 L2 Redis 二级缓存 (跟 eket `cache.rs` 1:1 借鉴, 跟 ioredis parity 1 项 联合) | Rule 4 (资源管理) + Rule 12 (3 模式) |
| **Gap 5** | KALLAX v3.5.0 实战 1 次 (commit `096eafe` line 22), 但 eket 实战多轮 (TASK-141 P0 Sprint1 + Round25 后), KALLAX 实战累计还 1:1 落后 eket | v3.6.0 候选: 增加 实战累计 (跟 eket Round25 实战路径 1:1 对齐, 跟 "诚实修正" 战略 联合) | Rule 18 (KPI falsification) + Q18 L4 (主公拍) |
...
- ❌ v3.5.0 hotfix 5 P0 finding 复发 5 release 累计 (跟 V310-B 反讽 1:1 复发 模式, 需 v3.6.0 持续 治根) — Gap 10 候选

**新 Gap (5 release 累计 反讽 1:1 复发, 跟 LESSONS-LEARNED-v3.5.0 §4.6 联合)**:
- Gap 11 (新增): v3.5.0 P-001 "eket parity 100%" 装饰反讽 跟 v3.1.0 P-002 "0 装饰引用" self-contradict 5 release 累计 反讽 1:1 复发, 需 v3.6.0 写 `scripts/verify/check-decorative-claim.sh` (跟 V310-B P-002 + V350-B P-001 + P-002 联合, 强制 evidence byte-different + `git grep | wc -l` 实测)
- Gap 12 (新增): v3.5.0 S-001 graceful-exit.sh fake theatre 跟 V310-B S-001 Slaver idle fake theatre 1:1 复发, 需 v3.6.0 写 signal handler 区分 SIGTERM (exit 143) 跟 SIGINT (exit 130) 强制验证
...
### KALLAX 5 release 累计 借鉴 实战 总结 (跟 LESSONS-LEARNED-v3.5.0 1:1 联合)
...
### 跟 V310-A / V350-A 1:1 联合 (本 angle 不重复 V310-A §4)
...
- V350-A 强项: `confluence/decisions/V310-A-REVIEW-2026-06-29.md` §6 强项 1-5 (跟 V350-A 5/5 维度 PASS 1:1 联合)
