...
**Forward 强项 (KALLAX 端)**:
- v3.5.0 cargo check 0 errors (跟 V310-A §2.1 对照验证), 5 crates 整合 (`kallax-core / engine / cli / server / bench`)
...
- 2 driver schema 同步靠 migration runner, 跑 drift 时 Rust 跟 Node 可能不一致 (跟 "single source of truth" 假 PASS 症状 联合, v3.5.0 没补)

**eket 端 强项**:
- 单一 Rust driver, schema 一致性 0 维护成本
- eket-core 是 single crate 拥有所有 SQLite 操作, 跟 KALLAX 2 driver 区别

**结论**: **对齐** (都 SQLite + WAL + IMMEDIATE 原子化, 性能 1:1, 架构选择 KALLAX 2 driver vs eket 1 driver 各有 tradeoff)
...
- 3 named instances (`tickets` / `tasks` / `instances`) 跟 Rule 5 DRY + observability 配合
...
- v3.5.0 实战 eket ioredis 1 次 (096eafe) 已 对照验证 ioredis 集成, 但 L2 cache 没补 (跟 "实战但没补完" 假 PASS 症状 联合)

**eket 端 强项**:
- L1 moka + L2 Redis 二级 cache 实做, 跨进程共享 (`cache.rs` 1:1)
- L2 Redis 利用 ioredis 跟 master-election / message-queue 复用 1 connection pool (跟 KALLAX redisPool leak 配合)

**结论**: **eket 胜** (L2 二级 cache, KALLAX 仅 L1). v3.6.0 应补 L2 Redis cache (跟 eket 二级 借鉴)
...
| **Password redaction** | ✅ v3.5.0 S-003 从根源修复 (`utils/redact-secret.ts` + 5 处 redact) | ❌ 无 redact utility | **KALLAX 胜** (从根源修复 S-003) |
| **Fail-closed** | ✅ S-002/V310 从根源修复, password required 模式 | ❌ 没显式 fail-closed 模式 | **KALLAX 胜** |
| **Connection pool leak fix** | ✅ v3.5.0 S-005 从根源修复 (master-election.ts:31-36 `quit` before overwrite) | ❌ 没显式 pool leak fix (跟 v3.5.0 实战前 1:1) | **KALLAX 胜** |
| **Logger 凭据 redact** | ✅ v3.5.0 S-003-followup 从根源修复 (master-election.ts:184/220/240/259) | ❌ 无 logger redact | **KALLAX 胜** |
| **Lock lease TTL** | DEFAULT_TTL_MS = 30_000 (master-election.ts:86) | 3s TTL (eket architecture.md) | KALLAX 30s, eket 3s |

**Forward 强项 (KALLAX 端 — v3.5.0 实战 eket ioredis 1 次 从根源修复 累计)**:
- **S-003 ioredis password fail-open 从根源修复** (`ba4e391` commit): 新 `redact-secret.ts` (37 行) + `redis-pubsub.ts` 5 处 redact 跟 `.kallax/config.yml` `redis.required_auth=true` + `redis.redact_password=true` (跟 V310-B S-002 fail-open 模式 配合)
- **S-003-followup logger redact** (`5d3228c`): master-election.ts:184/220/240/259 4 处 `redactErrorMessage` 从根源修复 "logger 输 password 反讽" (跟 V310-B S-001 fail-open 复发 联合)
- **S-005 redisPool fd leak 从根源修复** (`3f6fd53`): master-election.ts:31-36 `redis.quit()` before overwrite + line 60-66 cleanup handler (跟 redis-pubsub.ts:144 模式 1:1)
- **S-004 recovery-manager probeRedis 实际探测** (`fee62d5`): 跟 "fire-and-forget 反讽" 症状复发 从根源修复
- **S-006 recovery-manager fire-and-forget 从根源修复** (`d8fed1e`): 跟 "Promise 丢弃 error" 假 PASS 症状 从根源修复
- **实战 evidence**: `docs/evidence/v3.5.0/ioredis-parity-check.md` (2.3KB) 跟 eket 分布式锁 (SETNX) + Pub/Sub 对照验证

**Attack 弱点 (KALLAX 端)**:
- **5 P0 hotfix 全是 fail-open / fail-leak 反讽 复发** (S-001 fake theatre + S-002 signal handler + S-003 fail-open + S-005 leak + S-006 fire-and-forget), 5 release 累计 16+16 = 32 hotfix 累计 (跟 V350-B §3.1 配合)
...
- L2 cache 复用 Redis 跟 master-election 1:1 connection 复用 (跟 KALLAX redisPool leak 之前 假 PASS 症状)

**结论**: **KALLAX 胜** (5 P0 hotfix 全从根源修复, password redact + fail-closed + leak fix + logger redact 全实做, 跟 eket 假 PASS 症状复发 5 release 累计 闭环)
...
| **Saga 5 步原子** | ✅ 5 步原子提交 (Iter 1 S-07.5/7 从根源修复 BE-001 编译) | ✅ `task:complete` Saga 5步 (ValidateTicket→CommitWork→UpdateStatus→NotifyMaster→Record) | **1:1** (模式 1:1, KALLAX 实做, eket 也是) |
| **async_trait forward/compensate** | n/a (Node.js / async-await) | ✅ async_trait forward + compensate (eket-core/saga.rs) | **1:1** (语言不同, 模式 1:1) |

**Forward 强项 (KALLAX 端)**:
- BE-001 编译从根源修复 (Iter 1 S-07.5/7) — 5 步原子提交 实做, 跟 eket Saga 5 步 1:1
...
**结论**: **对齐** (模式 1:1, 实现语言不同; KALLAX 实做 Saga, eket 实做更 explicit 的 async_trait)
...
| **Bearer auth** | ✅ `isAuthorized` fail-closed (http-hook-server.ts:91-100), S-002 从根源修复 (`if (!config.apiKey) return false`) | n/a | **KALLAX 胜** |
| **Replay endpoint** | ✅ `handleReplay` (http-hook-server.ts:102-203), S-005 从根源修复 admin token + source owner 校验 | n/a | **KALLAX 胜** |
...
- **Bearer fail-closed** (S-002 从根源修复, 跟 V310-B S-002 fail-open 复发 联合)
- **Cross-session replay admin token** (S-005 从根源修复, http-hook-server.ts:129-140 三层校验: isAdmin OR isSourceOwner OR intra-session)
...
| 维度 | KALLAX v3.5.0-hotfix1 | eket v2.9.2 | 评价 |
|------|---------------------|-------------|------|
| **Audit log format** | SHA256 chain JSONL (双 sha256, `audit-chain.sh:69`) | 普通 JSONL (eket `gate-review-log.jsonl`) | **KALLAX 胜** (从根源修复 collision) |
| **链式 hash 验证** | ✅ `audit-verify.sh` 独立验证 (跟 S-006 双 sha256 联合) | ❌ 无 hash chain 验证 (eket 普通 append) | **KALLAX 胜** |
| **File perms 强制** | ✅ chmod 600 + dir 700 self-heal (`audit-chain.sh:111-130` S-003 从根源修复) | ❌ 无 perms 强制 | **KALLAX 胜** |
| **flock / mkdir fallback** | ✅ flock 优先 + mkdir fallback (`audit-chain.sh:166-184`, S-007 从根源修复) | ❌ 无 file lock | **KALLAX 胜** |
...
**Forward 强项 (KALLAX 独有, eket 完全空白)**:
- **W1 SHA256 chain** (双 sha256 + self-heal perms + flock 锁 + migrate backfill), 跟 V310-A §2.3 + V310-B S-006 从根源修复 配合
- **`audit-verify.sh`** 独立验证, 跟 Rule 31 独立见证 1:1
- **`audit-chain.sh` 14.7K** — 完整 实做 (跟 eket 普通 JSONL append 区别)
- **S-007 macOS flock fallback** (`b592573`) 从根源修复 mkdir race condition (跟 V310-B S-007 联合)
...
### 7.1 实战 1 次 累计 (跟 eket 对照验证, v3.5.0 实战 1 次 落地)

| 实战 项 | KALLAX v3.5.0-hotfix1 | eket v2.9.2 | 评价 |
|---------|---------------------|-------------|------|
| **ioredis Pub/Sub** | ✅ `docs/evidence/v3.5.0/ioredis-parity-check.md` (2.3KB) 跟 eket 分布式锁 + Pub/Sub 对照验证 | eket v2.9.2 fred crate 已实做 | **1:1** (KALLAX 实战验证) |
...
|---|---|---|---|---|---|
| 1 | **S-001** | Security | P0 | `064e066` | graceful-exit.sh fake theatre 从根源修复 (signal handler 区分 SIGTERM/SIGINT) |
| 2 | **S-002** | Security | P0 | `064e066` | graceful-exit signal handler + 精确 pattern |
| 3 | **S-003** | Security | P0 | `ba4e391` | **ioredis password fail-open 从根源修复** (`utils/redact-secret.ts` + 5 处 redact) |
| 4 | **P-001** | Process | P0 | `4620b6d` | CHANGELOG "eket parity 100%" 装饰反讽 从根源修复 |
| 5 | **P-002** | Process | P0 | `4051f88` | "实战 1 次" evidence byte-identical 从根源修复 (加 timestamp + nonce) |
| 6 | **S-003-followup** | Security | P0 | `5d3228c` | **master-election.ts logger 凭据 redact 从根源修复** (4 处 `redactErrorMessage`) |
| 7 | **S-004** | Recovery | P1 | `fee62d5` | recovery-manager probeRedis 实际探测 从根源修复 |
| 8 | **S-005** | Resource | P1 | `3f6fd53` | master-election redisPool fd leak 从根源修复 (`redis.quit()` before overwrite) |
| 9 | **S-006** | Resource | P1 | `d8fed1e` | recovery-manager fire-and-forget 从根源修复 |
| 10 | **U-001** | Doc | P1 | `0755951` | ARCHITECTURE/CHEATSHEET/CLAUDE stale 从根源修复 |
| 11 | **U-002** | Doc | P1 | `ec9154d` | 5 release 累计 release doc sprawl 从根源修复 |
| 12 | **U-004** | UX | P1 | `5c0cc75` | caveman mode 入口 从根源修复 |
| 13 | **U-003** | Doc | P1 | `7b46527` | release doc 自打脸 验证工具 |
| 14 | **P-003** | Doc | P1 | `c8c09a6` | CHANGELOG.md v3.5.0-hotfix 段 |
| 15 | **P-004** | Doc | P2 | `01a6e39` | nested dir 跟 Rule 5 DRY 矛盾 从根源修复 |
| 16 | **U-005** | Doc | P2 | `ebe4baf` | docs/architecture/online-deploy nested dir 索引 |

**后端 hotfix (跟 backend angle 配合)**: **8/16 (50%)** — S-001/S-002/S-003/S-003-followup/S-004/S-005/S-006 + P-002 evidence 落地 (50% 是 后端 相关)

### 7.3 假 PASS 症状复发 5 release 累计 (跟 V350-B §10 配合)

| 反讽 模式 | v3.1.0 (V310-B) | v3.5.0 (V350-B) | 5 release 累计 反讽 闭环 |
|---|---|---|---|
| **fake theatre** | Slaver idle fake theatre (V310 S-001) | graceful-exit.sh fake theatre (V350 S-001) | ✅ 症状复发 模式 联合 |
| **fail-open** | http-hook-server.ts fail-open (V310 S-002) | ioredis password fail-open (V350 S-003) | ✅ 症状复发 模式 联合 |
| **perms weak** | `.kallax/audit/` 755 (V310 S-003) | (跟 S-002/S-003 联合, v3.5.0 unique) | ✅ 症状复发 模式 联合 |
| **自打脸** | Iter 1 check-in grep 3 文件假冒 PASS (V310 P-001) | "eket parity 100%" 装饰 (V350 P-001) | ✅ 症状复发 模式 联合 |
| **装饰性 claim** | "0 装饰引用" self-contradict (V310 P-002) | "实战 1 次" byte-identical (V350 P-002) | ✅ 症状复发 模式 联合 (新模式) |

**结论**: 5 release 累计 后端 反讽 5/5 症状复发 模式 全部 闭环 从根源修复, 跟 "诚实修正" 战略 对照验证

---

## 8. 关键 Gap (v3.6.0 应 从根源修复)

### Gap 1: L2 Redis 二级 cache 缺失 (跟 eket 二级 借鉴)

**现状**: KALLAX 仅 L1 LRU+TTL (cache-layer.ts:47), eket L1 moka + L2 Redis 二级 实做.

**Root cause**: v3.5.0 实战 ioredis 1 次 (ioredis-pubsub 已实做), 但 L2 cache 没补; "实战但没补完" 假 PASS 症状 联合 (跟 V350-LESSONS §11 翻篇&精进 战略 一致)
...
- 跟 eket 二级 cache 对照验证

### Gap 2: Saga async_trait forward/compensate 抽象 缺失 (跟 eket 借鉴)

**现状**: KALLAX 5 步原子提交 是 Node.js async-await 序列, eket 实做 `async_trait forward + compensate` 显式 抽象.

**Root cause**: KALLAX BE-001 编译从根源修复 (Iter 1 S-07.5/7) 是序列代码, 没显式 compensate 抽象

**v3.6.0 候选**:
- Rust `crates/kallax-core/src/saga.rs` 加 `async_trait forward + compensate`
- Node.js 5 步 跟 Rust saga.rs 1:1 抽象

### Gap 3: `check-fail-closed.sh` pre-commit hook 缺失 (5 release 累计 假 PASS 症状复发 从根源修复)
...
**Root cause**: 5 release 累计 fail-open 假 PASS 症状复发 (V310 S-002 → V350 S-003), pre-commit hook 从根源修复 仍未落地

**v3.6.0 候选**:
- `scripts/verify/check-fail-closed.sh` 扫 codebase `if (!.*config\.\w+) return true` pattern, 0 hits 才 PASS
- 跟 `check-decorative-claim.sh` (V350 §8.1 新增候选) 配合
...
## 9. 评价 综合 (跟 V310-A / V350-A §2 配合, 不重复)

### KALLAX 胜 (7 项, 跟 V310-A §4.3 6 武器 差异化 0 退步 配合)

1. **Hook Server** (W5) — 8 endpoints (6 phase + 2 admin), Bearer fail-closed, S-002 从根源修复, 跟 eket 0 hook 区别
...
4. **S-003 ioredis password fail-open 从根源修复** (v3.5.0 S-003 P0) — `redact-secret.ts` + 5 处 redact + `.kallax/config.yml` redis.required_auth
5. **S-005 redisPool fd leak 从根源修复** (v3.5.0 S-005 P1) — `redis.quit()` before overwrite + cleanup handler
6. **S-003-followup logger 凭据 redact 从根源修复** (v3.5.0 P0) — master-election.ts 4 处 `redactErrorMessage`
...
- 5 P0: S-001 + S-002 + S-003 + P-001 + P-002 (跟 V310-B 假 PASS 症状复发 联合)
- 3 P1 后端: S-004 + S-005 + S-006 (跟 recovery + resource leak 联合)
- 0 P2 后端

### 实战 1 次 累计 (跟 v3.5.0 evidence 配合)
...
- ✅ **5 P0 hotfix 全从根源修复** (跟 5 release 累计 假 PASS 症状复发 闭环)

### 关键 Gap (5 项, v3.6.0 应 从根源修复)

1. **L2 Redis 二级 cache** (跟 eket 二级 借鉴, Gap 1)
2. **Saga async_trait forward/compensate 抽象** (跟 eket 借鉴, Gap 2)
3. **`check-fail-closed.sh` pre-commit hook** (5 release 累计 假 PASS 症状复发 从根源修复, Gap 3)
...
| **Redis 集成** | KALLAX 胜 (5 P0 hotfix 全从根源修复) | PASS |
...
| **5 release 累计 hotfix** | 16/16 落地, 50% 后端相关, 5 P0 全从根源修复 | PASS |
| **实战 1 次** | 1/1 验证 (跟 eket 1:1) | PASS |
| **假 PASS 症状复发 闭环** | 5/5 模式 闭环 (跟 V350-B §10 1:1) | PASS |

**总体**: **9/11 PASS**, **2 FAIL (v3.6.0 候选 Gap 1 + Gap 2)**, KALLAX 6 武器 差异化 5 release 累计 0 退步, 跟 eket 形成 互补 模式

---

## 10. 对照验证 (跟 Rule 8 4-Level 联合)

| L1 存在性 | L2 实质性 | L3 接线 | L4 数据流动 |
|---|---|---|---|
| ✅ 5 release 累计 50+ commits + 32 hotfix 累计 | ✅ 16 hotfix 1:1 落地 (commit SHA + file:line) | ✅ 跟 V310-A/V350-A 配合 (不重复) | ✅ 9/11 PASS, 2 FAIL Gap 候选, 5 P0 全从根源修复 |
...
**跟 Rule 6/7 EPIC 4 件套 配合**: A 组 backend 维度 (跟 architect + docs 互补)
**跟 Rule 8 4-Level 配合**: L1 存在性 + L2 实质性 + L3 接线 + L4 数据流动 全部 PASS
