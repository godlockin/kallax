...
> **范围**: 跨 EPIC,配合 (因为 EPIC-024 + EPIC-028 强耦合, tokenization 根本 bug + L1b 集成是同一 workstream).
...
- **Pe3 [MEDIUM] 决策者拍板 = 战略决策**: 决策者拍 A/B/C/D, Master 负责执行. 关键决策: C (REJECT + 严格重做) + 工具防御, B (扩词 + L1b 分线), D (Rust 重写) — 都是决策者, 不是 Master
...
- **Tool4 [EXISTING] `scripts/check-fact-forcing-preflight.sh`**: 5-Level 强制. 应集成 Tool1/2/3
...
- ✅ 战略 alignment 清晰: 飞轮路径基础 → 框架 → 扩展库 → 运行 → 迭代, 决策者拍板后 Master 落地无歧义
- ✅ 4 KPI 真实达标 (3 全 PASS, 1 borderline), 0 KPI 假数据
- ✅ 3 anti-fab 工具: 决策者要求 "加工具和限制", Master + 后续 Performer 实现
...
- **给 KALLAX Rule**: 升级 Rule 9 (5-Level Fact-Forcing) 加入 "KPI 估数算 FAIL" + "Scope creep 必拆 PR" + "Test case verbatim 触发" 检测

## 7. 跟其他 EPIC 的关联

- **EPIC-022 (PERMISSION v1)**: 强依赖 — 治理基础. EPIC-024 一切 routing 走 EPIC-022 authz
- **EPIC-023/025/026/027 (WORKFLOW 4 UP)**: 间接 — 5-Level 强制 / 4-Group A+B / L4 脚本存在 / LESSONS-LEARNED 强制. 本 EPIC 应用了这些规则, 也暴露了它们的不足 (估数 / scope creep)
