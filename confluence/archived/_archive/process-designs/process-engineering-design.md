...
> **归档 路径**: docs/_archive/process/process-engineering-design.md (如 需 进一步 归档)
...
| 方案 | 从根源修复程度 | 实施成本 | 跟 auditor 扩展组 联合 | 跟"激励扭曲" 联合 |
|---|---|---|---|---|
...
**决策**: 选方案 1 (独立见证机制) + 方案 4 (流程重构) 组合, 从根源修复 2 达到 90%.
...
## 3. 决策: 方案 1 + 方案 4 组合 (从根源修复 90%)

**跟"根因修复 2" 联合**:
- 方案 1 (独立见证机制): 从根源修复 100%
- 方案 4 (流程重构): 从根源修复 80%
- **组合**: 从根源修复 90%
...
| **BE-15** (3 假 PASS) | 3 假 PASS 0 commit | **Rule 30 + conductor-verify-gate (从根源修复 90%)** |

---

## 7. 总结 (跟"根因修复 2" + "流程逻辑" 战略 一致)

**4 方案对比**:
- 方案 1 (独立见证机制): 从根源修复 100%, 高实施成本
- 方案 2 (双人审计): 从根源修复 60%, 中实施成本
- 方案 3 (强制 FAIL 奖励): 从根源修复 40%, 低实施成本, 易被滥用
- 方案 4 (流程重构): 从根源修复 80%, 中实施成本

**决策**: 方案 1 + 方案 4 组合, 从根源修复 90%.
...
**commit 准备**: 跟 miao HEAD `703aa93` 一致, 跟"根因修复 2" + "流程逻辑" 战略 一致