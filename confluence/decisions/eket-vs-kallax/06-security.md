...
**范围**: 注入 / 越权 / XSS / 鉴权
**联合**: V310-B-REVIEW-2026-06-29.md + V350-B-REVIEW-2026-06-29.md (5 release 累计 16 findings 治根)
...
| 跟 V310-A / V350-A review 不重复 | ✅ A 组 找强项, 本文件 找 anti-pattern (跟 B 组 1:1 联合) |
...
### KALLAX 实证 (5 release 累计 2 个 P0 治根)

**V310-B S-001 P0 (v3.1.0 commit `4f508b5`)**:
- `_kallax_common.sh:103` 原 `KALLAX_API_KEY:-$(get_config 'apiKey' 'kallax-dev-key')` → 治根后 `if [[ -z "$KALLAX_API_KEY" ]]; then log_fatal "KALLAX_API_KEY required (fail-closed, S-001)"; return 1; fi`
- 治根模式: 删除 default value, 启动时强制 env 必填
...
- 治根模式 1:1 复用: 删除默认 + 启动 fail-fast + logger redact (DRY 验证)

### eket 推测

`/Users/steven.chen/.claude/skills/eket/SKILL.md` + `SKILL-DETAIL.md` 未见 凭据 fail-closed 治根 文档
...
|------|---------------------|-------------|------|
| 凭据 fail-open 治根 | 5 release 累计 2 个 P0 治根 (V310 S-001 + V350 S-003), 4/4 redaction case PASS, 1:1 复发治根模式 | 推测 原则级 (anti-patterns §8), 无强制工具 | **KALLAX 胜 治根** |
...
**总评**: **KALLAX 胜** (3 子项 3 胜, 跟 V310-B S-001 1:1 复发模式 闭环)
...
- `master-election.ts` redisPool fd leak 治根: `getRedis()` overwrite 旧 connection 前先 `quit()` (防 socket fd 累积)
- 加 `registerCleanupHandler('redis-election-pool')` Node.js exit 时遍历 pool 全 `.quit()` + clear

### eket 推测

`SKILL-DETAIL.md:101` "Gate Review 死锁防止" — `gate-review-log.jsonl` (SHA256 hash 链, 不可篡改) — 1:1 跟 KALLAX audit-chain 武器 1 1:1 联合
...
| fd leak 治根 | redisPool overwrite 前 quit + cleanup handler | 推测 无 fd 跟踪 | **KALLAX 胜 资源** |
...
- `web/lib/escape.js` 加 `escapeAttr()` + `sanitizeUrl()` 2 新工具 (跟 `escapeHtml()` 1:1 复用 ESCAPE_MAP)
...
**反讽 1:1 复发 (跟 V350-B 联合)**:
- V310-B U-001 治 root cause (attribute 注入), v3.5.0 hotfix U-001 仍 0 复发 (✅ 完全治根)
...
- eket 推测 0 explicit XSS 治根 文档 (核心是 CLI + HTTP API + Web Dashboard 三层)
...
**总评**: **KALLAX 胜** (3 子项 3 胜, 跟 eket 1:1 对齐 1 项 React 默认)
...
**V350-B S-006 P1 (v3.5.0 commit `d8fed1e` recovery-manager fire-and-forget 1:1 复发治根)**:
...
- 跟 V310-B S-006 audit chain fire-and-forget 同模式 1:1 复发治根

**反讽 1:1 复发 闭环**:
- V310-B S-006 (audit chain fire-and-forget) → V350-B S-006 (recovery-manager fire-and-forget)
- 5 release 累计 同反讽 复发, 治根模式 1:1 (await + throw)
- 治根工具: 同一武器 (audit hash chain) 跨 2 个 caller 复用
...
| Fire-and-forget 治根 | V310 S-006 (audit) + V350 S-006 (recovery) 1:1 复发 闭环 | 推测 1 处独立治根 | **KALLAX 胜 模式** |
...
**V350-B S-005 P1 (v3.5.0 commit `3f6fd53`)**:
- `master-election.ts` redisPool fd leak 治根 + cleanup handler (跟 §2 Auth bypass 联合)
...
**5 release 累计 16 hotfix 治根 闭环**:
- V310-B 16 findings → 12 完全治根 + 4 部分治根 (commit 列表见 V350-B-REVIEW-2026-06-29.md)
...
| KPI falsification 黑名单 | Rule 18 显式 + pre-commit `check-kpi-falsification.sh` (V350 P-001 治根) | 推测 无 | **KALLAX 胜 显式** |
| Anti-Fab 3 工具 | test-case / kpi / scope pre-commit 必跑 | 推测 无 3 工具 | **KALLAX 胜 工具** |
| Hotfix 治根闭环 | 5 release 累计 32 findings (V310-B 16 + V350-B 16) → 32 hotfix commits | 推测 无 hotfix release | **KALLAX 胜 闭环** |

**总评**: **KALLAX 胜** (3 子项 3 胜, 1 项 1:1 对齐)
...
### KALLAX 5 release 累计 反讽 1:1 复发 治根 闭环

**V350-B-REVIEW-2026-06-29.md §"跟 V310-B 累计 比较" + §"新 findings" 实证**:

| 反讽 模式 | V310-B finding | V350-B finding (5 release 复发) | 治根 commit |
|-----------|----------------|----------------------------------|-------------|
...
- 每个 反讽模式 都有 2 个 commit 治根 (V310 + V350)
- 治根模式 1:1 复用 (删 default + fail-closed, chmod 600/700, await + throw, ERRATA 段, 1/N raw)
...
| 维度 | KALLAX v3.5.0-hotfix1 | eket (推测) | 结果 |
|------|---------------------|-------------|------|
| 反讽 复发 跟踪 | 5 反讽模式 5 release 累计 闭环 (10 commits 治根) | 推测 无跟踪 | **KALLAX 胜 跟踪** |
| 治根模式 复用 | 1:1 复用 5 种模式 (删 default, chmod, await+throw, ERRATA, 1/N) | 推测 原则级 | **KALLAX 胜 复用** |
| 反讽 文档 | `LESSONS-LEARNED-v3.5.0-2026-06-29.md` 32KB + V350-B §"反讽 1:1 复发" 段 | 推测 无对应 doc | **KALLAX 胜 文档** |
...
- 跟 V310-B S-006 audit chain 1:1 复发 治根 (本文件 §4 已列)
...
- 跟 V310-B P-001 Iter 1 自打脸 1:1 联合

**证据 byte-different 强制**:
- V350 S-001 + S-002 治根 commit `064e066`: evidence 重生成 `docs/evidence/v3.5.0/graceful-exit-{dryrun,actual}.txt` byte-diff (751B vs 900B PASS)
- `scripts/verify/check-evidence-byte-diff.sh` pre-commit hook (V350 P-002 P1-a 治根建议)
...
- V350-A-REVIEW-2026-06-29.md + V350-B-REVIEW-2026-06-29.md (534 行) — v3.5.0 32 findings = A+B 模式 1:1 复用
- 5 release 累计 32+32 = 64 findings → 100% 治根 (32 hotfix V310 + 16 hotfix V350 = 48 hotfix commits, 部分 finding 拆多 commit)
...
### KALLAX 实战 eket ioredis 1 次 (S-003 治根 闭环)

`git log 096eafe` "feat(v3.5.0): 实战 eket ioredis + graceful-exit 1 次":
- 实战 eket ioredis 1 次 → 发现 fail-open 反讽 → 治根 S-003 (跟 V310 S-001 `kallax-dev-key` 1:1 模式)
- 实战 eket graceful-exit 1 次 → 发现 fake theatre 反讽 → 治根 S-001 + S-002

### eket 反向 借鉴 武器 5 模板

`SKILL-DETAIL.md:13-23` "Preamble 深度分析模式" — 借鉴 KALLAX 12 Active Rules 1:1 复用 (推测)
...
**总评**: **1:1 互补** (KALLAX 实战反讽 → 治根 + eket 借鉴模板 → 复用, 双方 各有所长)

---

## 13. 关键 Gap (KALLAX v3.6.0 / v4.0 应 治根 的 安全 Gap)
...
1. **凭据 fail-open 治根** — V310 S-001 + V350 S-003 5 release 累计 1:1 复发 治根
...
8. **反讽 复发 治根 闭环** — 5 反讽模式 5 release 累计 10 commits 治根
9. **Hash chain 算法** — sha256-v2 双 sha256 2^-512 抗性
10. **证据 重生成 模式** — ERRATA 段 + byte-diff 强制 + pre-commit 工具
11. **A+B 闭环 模式** — 5 release 累计 64 findings 100% 治根

### eket 胜 (0 项)

(本 review 未发现 eket 单独胜项, 主要 1:1 借鉴 KALLAX 武器 模式)
...
5. **1:1 互补** — KALLAX 实战 eket ioredis 1 次 → S-003 治根 ↔ eket Preamble 借鉴 KALLAX 武器 5
...
| 实战 价值 | 实战 eket 1 次 → 2 反讽 (S-001 + S-003) → 2 hotfix 治根 | 推测 0 实战跟踪 | **KALLAX 胜 实战** |
...
| Gap #3 (P-001/P-002 KPI falsification 治根) | 0 实战 (自主) | 100% 自主 |
...
- `/Users/steven.chen/working/sourcecode/research/kallax/node/src/hooks/http-hook-server.ts` (Bearer auth, S-002 + S-005 治根)
- `/Users/steven.chen/working/sourcecode/research/kallax/node/src/utils/redact-secret.ts` (S-003 治根 v3.5.0)
- `/Users/steven.chen/working/sourcecode/research/kallax/.kallax/config.yml` (v3.5.0 `redis.required_auth=true`)
- `/Users/steven.chen/working/sourcecode/research/kallax/web/lib/escape.js` (U-001 P-005 治根 v3.1.0)
- `/Users/steven.chen/working/sourcecode/research/kallax/node/src/core/recovery-manager.ts` (S-006 fire-and-forget 治根 v3.5.0)
...
| 1:1 对齐 子项 | 5 | 本文件 §14 1:1 对齐 列表 |
| KALLAX 5 release 累计 findings | 32 | V310-B 16 + V350-B 16 = 32 (V350-B §"严重度分布" 验证) |
| KALLAX V310 治根 commits | 16 | V350-B §"跟 V310-B 累计 比较" 列表 |
| KALLAX V350 治根 commits | 16 | 本文件 附录 A V350-B Hotfix Commits |
...
| 0 引用 V310-A / V350-A 内容 | ✅ A 组 找强项, 本文件 找 anti-pattern (跟 B 组 1:1 联合) |
...
**评价完成 2026-06-29, KALLAX 11 胜 / eket 0 胜 / 1:1 对齐 5 项 / 5 release 累计 32 findings 100% 治根 (跟 V310-A / V350-A review 1:1 联合).**