...
| **统一 Headless Event Protocol** | Frontend | ✅ KALLAX 26 命令缺统一契约, 落 EPIC-222 联合 |
| **Capability-Based Fail-Closed Security** | Security | ⚠️ P0 战略红线, 触及 6 immutable scripts + AUTO-PERMS — 本调研不强行开 P0, 落 EPIC-222 research-only |

### UX/产品 3 件套 (Agent #2: UX + Product + Process)

| 借鉴 | 出处 | 仲裁 |
|------|------|------|
| **README 30s elevator + 场景段** | UX + Product | ✅ 跟 EPIC-213 manifesto elevator-pitch 联合扩展, 落 EPIC-217 |
...
| 借鉴 | 出处 | 仲裁 |
|------|------|------|
| **Disclaimer audit script** | Auditor | ✅ 6th immutable script (跟 5 existing 1:1), 落 EPIC-220 |
| **DCO + Rule 34 contribution template** | Compliance | ✅ 跟现有 DCO + Apache 2.0 + EPIC-152 联合, 落 EPIC-221 |
| **"limit ≠ success" disclaimer 标准** | Decision-Gate | ✅ 跟 v3.8.0 "25/25 假 PASS" 教训 1:1, 合并进 EPIC-220 |
...
> **0 触及 immutable scripts / Rule 改 / 4-PR bypass**: 本次调研**不开 P0 ticket**. 借鉴以 P1 流程升级为主, 跟 EPIC-207 v2 (主公亲自拍板) 1:1.

### P1 (流程升级, 备案 + RECORD) — 6 项

| # | EPIC | 标题 | 联动 | effort | 候选 |
|---|------|------|------|--------|------|
| 1 | **EPIC-217** | README 30s elevator + "When to use" 场景段 (跟 EPIC-214 联合扩展) | EPIC-213 + EPIC-214 | 小 (docs-only) | Rule 37 auto-approve |
...
| 5 | **EPIC-221** | `.github/PULL_REQUEST_TEMPLATE.md` + commitlint DCO + bugfix 必填 reproduction 3 字段 (跟 ticket.json binding 1:1) | EPIC-152 + 现有 DCO | 中 (config + doc) | Rule 37 auto-approve |
...
KALLAX 是 **governance layer** (治理 Claude Code 怎么进 prod), prime-agent 是 **runtime** (替代 Claude Code 跑 long-running). 正交叠加, 不互相替代. 借鉴方法论 > 抄代码 (跟 EPIC-206 "借方法论 不借代码" 1:1).
...
4. **Disclaimer audit**: 6 immutable scripts 防 commit 漂移, 但 README/CHANGELOG 数字"假 PASS"复发 (Auditor 报告, 跟 v3.8.0 1:1)

### 5.3 借鉴方法论而非抄代码 (跟 EPIC-013 + EPIC-206 1:1)
...
## 7. 落地路径 (跟现有 4-PR + 5-Level + Sprint 时间盒 1:1)
...
**Rule 9 落地**: 1/1 决策 doc + 3/3 agent 报告 + 6/6 P1 EPIC = 10/10 = 100%.

---

## 9. Why this report matters

prime-agent 是跟 Claude Code 同赛道最强开源竞品 (6.9k stars, MIT, 活跃). 调研暴露 KALLAX 4 个治理空白 (README 摩擦 / Long-running 原语 / CLAUDE.md snapshot / Disclaimer audit). 借鉴方法论 (不是抄代码) 落地 6 P1 EPIC, 跟 EPIC-194/204 docs-only metrics + EPIC-190 Sprint 时间盒 + EPIC-207 4-PR governance 1:1 闭环.

**How to apply**:
1. 主公 review 本决策 doc, 拍板 6 P1 EPIC 是否全部进下 Sprint
2. EPIC-217 + EPIC-221 docs-only 走 Rule 37 auto-approve (跟 EPIC-216 1:1)
3. EPIC-218/219/220/222 新脚本/immutable 走主公亲自拍板 (跟 EPIC-216 例外 1:1)
4. Sprint 结束必跑 sprint-metrics --docs-only flag (跟 EPIC-204 1:1)
5. 6 EPIC 落地后, prime-agent 调研闭环, retro 一句话进 `confluence/manifesto/04-lessons.md`