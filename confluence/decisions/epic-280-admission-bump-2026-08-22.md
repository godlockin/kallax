...
- **B 组红蓝对抗 blocker**: 缺 confluence/decisions/ 书面决策记录 → 本 doc 闭环
...
9/9 PASS  → 10/10 PASS (verify-agent-note-format.sh 接通)
```
...
| EPIC | 关系 |
|------|------|
| EPIC-225 check-jargon.sh | 1:1 接入模式 (canonical+fallback + install --verify + pre-commit) |
| EPIC-279 check-doc-budgets.sh | 同模式 (1 天前刚合) |
| EPIC-223 check-ticket-schema.sh | 同样是 admission 历史先例 |
| EPIC-277-E (2026-08-21) | hooks/ 接入 4 新 + install --verify 9/9 PASS 验证机制 |
| EPIC-241 | bypass 备案 pattern 1:1 复用 |

## 5. 后续债

- **bypass 形成 anti-pattern**: 连续 #475 + #479 2 单 (沿用 EPIC-241 备案同)
- **跨 EPIC 复用率**: 4 EPIC (280/281/282/283) 约 65% (PASS Rule 36 #2)
