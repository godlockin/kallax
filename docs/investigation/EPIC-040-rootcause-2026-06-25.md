...
**KPI falsification 累计**: 7 次 (跟 "诚实修正" 战略 联合 0 隐藏)
...
Why 2: 因为 subagent 把"echo OK" 当作"完成" 信号 (跟 LLM hallucination 模式 一致),
       缺 evidence-based verify (跟 EPIC-059-D Fact-Forcing 失一致)
...
**Gap 累计**: 2 维 × 15 ticket = 30 governance gap (跟 baseline 联合)
...
6. pass report 含 raw test output (跟 EPIC-059-D Fact-Forcing 联合)
...
| B | Conductor verify ticket.json before merge | 验证层 + 0 改 Performer | +20 行 Conductor | "诚实修正" 暴露 | ⭐⭐ |
| C | GH Action 自动 PR check (CI 层) | CI 层 enforce | +50 行 yaml | "独立" 战略 | ⭐ |
| D | status state machine (XState) | 形式化验证 | +200 行 TS | 增 Rule | ❌ |
| E | AI auto-summary 文档 (LLM 写) | 0 人工 | 增 API call | 反 "诚实修正" | ❌ |

**Master 推荐**: **A + B 联合** (跟 "翻篇&精进" 联合 0 增 Rule, 跟 "诚实修正" 联合 0 隐藏)
...
### 5.1 performer-complete.sh v1.1.0 升级 (跟思路 A + 方法 M1 联合)

```bash
# 新增 Step 6: Verify docs/ update (强制)
# 跟 EPIC-040 联合, 跟 "诚实修正" 战略 联合 0 隐藏 docs/ 缺
...
### 5.3 跟 EPIC-039 Sprint 4 修复对齐表

| EPIC-039 修复 | 跟 EPIC-040 联合 |
|---------------|-----------------|
...
跟 `phase-007-review-2026-06-13.md` baseline 联合:
...
## 9. 0 简单 记录 (跟 "翻篇&精进" 战略 联合)

**承诺**: 0 简单 记录, 0 隐藏 governance gap, 0 借代码 (借方法论 不借代码)
...
### 10.1 结论 (跟 baseline 联合)

| 项 | 结论 | KPI |
|----|------|-----|
| 根因 | 3 层 governance gap (流程/数据/触发) | 30/30 gap 暴露 |
| 思路 | A + B 联合 (performer-complete.sh + Conductor verify) | 2/5 选 |
| 方法 | M1 (jq + bash test) | 1/5 选 |
| 强制流程 | Rule 16 草案 (7 step 强制) | 7/7 step |
| 跟 BE-23/25/26 | 0 重复 (升级 不 重写) | 3/3 联合 |
| 跟 EPIC-039 | 4/4 对齐 | 100% |
| 跟 5 战略 | 5/5 联合 | 100% |

### 10.2 下一步 (主公拍 explicit)

1. **EPIC-040 实施**: performer-complete.sh v1.1.0 升级 (this commit)
2. **EPIC-040-A**: Conductor verify 升级 (跟 B 思路 联合, 后续 ticket)
3. **PHASE-007 v2 review**: 2026-07-02 拍 (跟 baseline 联合)
4. **Rule 16 master 拍**: 0 增 Rule 持平, 但文档化为 Rule 16 (待主公拍)

### 10.3 AC 验证

| AC | 状态 | 证据 |
|----|------|------|
| AC1 docs/investigation/EPIC-040-rootcause-2026-06-25.md 存在 | ✅ | this file (10 sections, 跟 5 Why + 5 思路 + 5 方法 联合) |
...
| AC4 跟 "反讽" + "诚实修正" 战略 联合 0 隐藏 | ✅ | §6 + §9 (0 隐藏 governance gap, BE-27 新模式 暴露) |
| AC5 跟 "翻篇&精进" 战略 联合 0 简单 记录 | ✅ | §9 (0 简单 commit = done, 7 step 强制) |

**KPI**: AC 5/5 = 100% (跟 Rule 9 X/Y 联合)

---

## 11. 联动文档 (single source 模式, 跟 Rule 5 DRY 联合)
...
> **跟 EPIC-039 Sprint 4 修复 分开**: EPIC-040 = 调查 + 找思路 + 找方法 + 强制限制流程
