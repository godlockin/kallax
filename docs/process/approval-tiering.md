# 决策者拍板分级 P0/P1/P2 — 决策疲劳从根源修复 (EPIC-055-B)

> **Ticket**: EPIC-055-B (5 张治理卡 核心)
> **Date**: 2026-06-16
> **Author**: performer-EPIC-055-B
> **Status**: ✅ DESIGN — 跟决策者 2026-06-16 拍板,配合, 跟 PROCESS.md:25-26,配合

---

## TL;DR

**问题**: 决策者决策疲劳 — 23 Rule 累计 9 升级, 拍板边际效用递减, ai-copilot 模式 "ask every step" (每 5 分钟 1 次确认请求).

**方案**: 决策者拍板 3 级分类:
- **P0** 战略红线 (R-NEW 升级 / Rule 撤销 / 治理升级) — **必拍**, subagent 阻塞等决策者
- **P1** 流程升级 (Tier 1/2 ticket / Rule 合并 / 阶段变更) — **备案**, 决策者 review 即可
- **P2** 操作 (Tier 3 chore / docs / 单文件改动) — **放手**, 不需备案, 直接执行

**Rule 33** (新): 决策者拍板分级 P0/P1/P2 (软限制, 跟 Rule 11 v2.1 强验证 联动)

---

## 1. 根因 (跟 23 Rule 9 升级,配合, 跟决策疲劳,配合)
...
| 升级率 | 39.1% | 9/23 |
| ai-copilot 决策者确认频率 | 每 5 分钟 1 次 | docs/process/decision-gate-design.md §1.1 |
| 决策者决策疲劳度 | 高 (P2 操作 仍需拍板) | decision-gate-design.md §1.5 UX 视角 |

### 1.2 根因

PROCESS.md:25-26 明确: **Master 不能自己升级红线**, 治理升级需决策者 explicit 拍板.
但没区分拍板级别 → 所有决策都走"必拍"路径 → 决策者 P2 操作 (docs typo / chore) 也需拍板 → 边际效用递减.
...
## 2. 3 级分类设计 (跟 PROCESS.md:25-26,配合, 跟"独立" 拍 explicit 约束,配合)

### 2.1 P0 — 战略红线 (必拍)

**范围**:
- R-NEW 升级 (新 Rule 制定, 配合 v1.2.4 5 扩展组 联动)
- Rule 撤销 / 合并 (配合 EPIC-054-D 联动)
- 治理升级 (改 KALLAX 核心机制, 5 治理卡 类)
- Tier 0 ticket (战略 ticket)
- 红线 revert (推翻 决策者 prior 拍板, 如 EPIC-056-C)

**拍板方式**: **阻塞等决策者 explicit 拍板**
- subagent 不能自助执行 (跟 PROCESS.md:25-26 Master 不能自己升级红线,配合)
- 写 `inbox/human_feedback/REQUEST-P0-<id>.md` (拍板请求)
- 决策者拍板 后 → subagent 执行 + 更新 audit + 通知 Conductor

**决策者成本**: 高 (一次性, 15 分钟/次 估算)
...
**拍板方式**: **写 inbox 备案, 决策者 review 即可**
- subagent 不阻塞, 直接执行
- 写 `inbox/human_feedback/RECORD-P1-<id>.md` (备案)
- 决策者 review 时 check 即可 (不阻塞)

**决策者成本**: 中 (5 分钟/次 估算, 仅 review 不阻塞)
...
```bash
route_p0()  # → 阻塞 + 写 REQUEST-P0-*.md (决策者 explicit 拍板)
...
## 4. 拍板疲劳从根源修复 (跟 Rule 32 联动, 配合 EPIC-054-D 联动)
...
- `P0_MISSED: N P0 漏拍 (scanned M tickets)` — 需决策者补拍
...
派单顺序 (跟 `5-GOVERNANCE-CARDS-APPROVAL-2026-06-16.md`,配合):
...
## 7. 跟 PROCESS.md:25-26,配合 (联合红线)

PROCESS.md:25-26 红线: **Master 不能自己升级红线**

3 级分类 **不** 突破 PROCESS.md:25-26 红线:
- ✅ P0 必拍 (跟红线一致)
- ✅ P1 备案 (不阻塞, 但留痕, 决策者可 review)
- ✅ P2 放手 (操作类, 不涉及红线升级)
- ❌ Master 永远不能自助 P0 (红线不变)

**验证**: role-cmd.ts 升级 后, route_p0 仍需决策者 explicit 拍板 才执行.

---

## 8. 跟"诚实修正评估",配合

决策者 2026-06-16 一次性拍 5 张治理卡, 跟"独立" 拍 explicit 约束,配合.
...
| 4 | P2 从根源修复 — 拍板疲劳 完整完成 | TC6 fatigue_index + recommendation |
| 5 | 6/6 PASS | 测试输出 6/6 |
| 6 | Rule 9 KPI 精确 X/Y 格式 | 6/6 = 100.0% |
| 7 | 跟决策者拍板,配合 | 引用 5-GOVERNANCE-CARDS-APPROVAL-2026-06-16.md |

---

**跟 5-GOVERNANCE-CARDS-APPROVAL-2026-06-16.md,配合, 跟 PROCESS.md:25-26,配合, 跟 Rule 9 X/Y 格式,配合, 跟 Rule 11 v2.1 强验证,配合, 跟 Rule 32 联动, 配合 EPIC-054-D/056-A/056-C 联动, 跟"诚实修正评估" + "翻篇&精进" 战略 一致**