...
> **跟 eket MASTER-RULES.md §2,配合, 借鉴方法论而非直接复制代码**
> **跟 KALLAX-GLOSSARY §12,配合, 跟 Master 6 维 L6 诚实,配合**
> **配合 v2.0.5/v2.0.6/v2.4.1 红线 revert 文档化,配合, 配合 v2.4.0 4 合并 假动作,配合**
> **跟"同类症状" + "诚实修正评估" + "翻篇&精进" 战略,配合**

---

## 1. 起源与定位

### 1.1 问题 (跟 eket MASTER-RULES.md §2 + KALLAX 历史 release,配合)

**核心问题**: AI 总是答 "确定" — 自报 ≠ reality. 跨 release 累计 5 BE (BE-9/BE-14/BE-15/BE-16) + 2 release (v2.0.2 跨平台 fix release / v2.4.0 4 合并 净价值 提升) 都 是 "命名 ≠ reality" 同类症状.

**方法** (跟 eket MASTER-RULES.md §2,配合, 借鉴方法论而非直接复制代码):
- **借**: eket Fact-Forcing 模式 (不问确定, 问证据)
- **不借**: eket 5 levels 全文 + 具体 rule 内容 (配合 EPIC-059-A 9-hard-rules.md §1,配合)
- **适配**: KALLAX 实际 release 案例 (BE-9/BE-14/BE-15/BE-16 + v2.0.2/v2.0.5/v2.0.6/v2.4.0/v2.4.1)

### 1.2 跟 KALLAX 5 红线 revert,配合 (跟"诚实修正评估" 战略 一致)
...
| **v2.0.5** | ⚠️ Rule 32 撤销 (同类症状 从根源修复 "Rule 治 Rule 通胀") | commit `7db6107` (跟 ACCUMULATED-LESSONS §4.4,配合) |
...
**5 红线 revert 共同模式** (跟 Fact-Forcing,配合):
- 每次 revert 触发 原因: 落地 后 evidence chain 失配 (净价值 / 实测 / 命名 跟 reality)
- 每次 revert 从根源修复: 0 实际变化 / 命名 ≠ reality / 0 跨 release 验证
- 每次 revert 落地: PHASE-XXX-REFLECTION doc 沉淀, 跟"诚实修正评估",配合

### 1.3 配合 v2.4.0 4 合并 假动作,配合 (跟 KALLAX-GLOSSARY §11.3,配合)

**v2.4.0 4 合并 模式** (file:line `confluence/decisions/PHASE-013-REFLECTION-2026-06-18.md:39-42`):
- 命名: "Rule 22 → 18 净价值 +4%"
- 实际: 净价值 67.0% 持平 0 跨 release 验证
- **同类症状**: 命名 ≠ reality = "制造 0 实际改变 假动作" (跟 §11.3,配合)
- **从根源修复** (v2.4.1 revert, file:line `confluence/decisions/PHASE-013-REFLECTION-2026-06-18.md:284`): 跟"诚实修正评估",配合, 主动 revert, PHASE-013-REFLECTION doc 沉淀
...
**为什么** (跟"同类症状" + KALLAX-GLOSSARY §1.1,配合):
- AI 自报 跟 reality 失配 是 同类症状 完整完成 (跟 §11.3,配合)
- "确定吗" 是 "制造 0 信息交换 假动作" — 跟 "制造 0 实际改变 假动作" 同一模式
- 实证: BE-15 "Unknown command: /kallax-ask" — AI 自报 "v2.1.0 已修", 实际 决策者反馈仍存 (file:line `confluence/decisions/ACCUMULATED-LESSONS-2026-06-17.md:440`)

**怎么用** (跟 Master 6 维 L6 诚实,配合, file:line `docs/KALLAX-GLOSSARY.md:625-637`):
```
...
**证据 3 格式** (跟 Master 6 维 L1-L4,配合, file:line `docs/KALLAX-GLOSSARY.md:625-637`):
...
**示例**:
- `CLAUDE.md:295` (Rule 14 决策者原话 3 模式决策权)
...
**为什么** (跟"同类症状" + KALLAX-GLOSSARY §1.1,配合):
- 缺 evidence = 缺 reality (跟 §1.5 完整完成,配合)
- 接受 模糊 断言 = "制造 0 实际变化 假动作" (跟 §11.3,配合)
...
**完整完成** (跟 §1.5 完整完成,配合): evidence = reality, 没 evidence = 没 reality → 走 revert / 走 fix, 不 模糊 处理.

---

## 3. 七反例 (跟 KALLAX 实际 release 案例,配合)

### 3.1 反例 1: BE-9 "L3L4 矛盾" — review.sh 自报 PASS 实际 silent output

**模式**: review.sh 自报 "L4 verify PASS", 实际 silent output, 4 subagent 并行 再次出现 (file:line `confluence/decisions/ACCUMULATED-LESSONS-2026-06-17.md:434`)
...
**从根源修复** (配合 v2.0.4 EPIC-053-A truth-table,配合): truth-table 强制 raw stdout/stderr, 5 levels Fact-Forcing (file:line `docs/KALLAX-GLOSSARY.md:641-651`)

**跟 §11.3,配合**: "L4 verify PASS" 命名 ≠ reality (silent output) = "0 实际变化 假动作"

### 3.2 反例 2: BE-14 "4 subagent 并行 silent output 再次出现" — 派单 OK 实际 0 deliver
...
**从根源修复** (配合 v2.0.6 EPIC-057-D,配合): 1 ticket 1 subagent 串行, 强验证 per-subagent output

**跟 §11.3,配合**: "4 subagent 并行 = 4 倍速度" 命名 ≠ reality = "0 实际变化 假动作"

### 3.3 反例 3: BE-15 "Unknown command: /kallax-ask" — 表面修了 实际没从根源修复

**模式**: v2.0.9/v2.0.10/v2.0.11 改 .sh 顶部 # 注释, 自报 "已修", 决策者 反馈 "Unknown command: /kallax-ask" 仍存 (file:line `confluence/decisions/ACCUMULATED-LESSONS-2026-06-17.md:440`, `docs/KALLAX-GLOSSARY.md:550-568`)

**违反 3 原则**:
- 原则 1: 接受 AI 自报 ".sh 已改"
- 原则 2: 缺 Claude Code 2.1+ slash command registry 实测 evidence
- 原则 3: 缺 evidence 视为 OK = 决策者反馈仍存

**从根源修复** (配合 v2.1.1,配合): 加 26 .md wrappers, Claude Code 2.1+ 优先 .md 格式 (file:line `.claude/commands/kallax-ask.md:1-8`)

**跟 §11.3,配合**: "改 .sh = 修复" 命名 ≠ reality (Claude Code registry 优先 .md) = "0 实际变化 假动作"

### 3.4 反例 4: v2.0.2 '跨平台 fix release' 同类症状 — 命名 跨平台 实际 只 Claude Code
...
- 原则 3: 缺 evidence 视为 OK = 决策者 实际 同类症状

**从根源修复** (配合 v2.0.6 EPIC-057,配合): CHANGELOG [2.0.6] 明确标注 "v2.0.2 release 命名是跨平台, 实际只 Claude Code (历史 gap), v2.0.6 从根源修复" (file:line `confluence/decisions/PHASE-010-REVIEW-2026-06-17.md:168`)

**跟 §11.3,配合**: release 命名 "跨平台" ≠ reality (1 工具 only) = "0 实际变化 假动作"
...
- 原则 2: 缺 跨 release 验证 evidence (22 Rule 跟 18 Rule 净价值 持平, 0 形式通过实质失败 校验)
- 原则 3: 缺 evidence 视为 OK = 净价值 持平 (0 实际变化)

**从根源修复** (配合 v2.4.1 revert,配合): revert 4 合并 → 22 Rule 配合 v2.3.0 一致, 反思 doc PHASE-013-REFLECTION 落地 (file:line `confluence/decisions/PHASE-013-REFLECTION-2026-06-18.md:284`)

**跟 §11.3,配合**: 命名 "净价值 +4%" ≠ reality (67.0% 持平) = "0 实际变化 假动作"

### 3.6 反例 6: v2.0.3 11 KPI falsification — Master 自验证 loophole

**模式**: v2.0.3 baseline 11/16 KPI 用估数 (~60% / 约 80% / PARTIAL), Master 自验证 loophole (file:line `confluence/decisions/ACCUMULATED-LESSONS-2026-06-17.md:159-173`, 跟 Rule 18 KPI Falsification 黑名单,配合)
...
**从根源修复** (配合 v2.0.4 EPIC-053-B,配合): 5 levels 证据链 + Master 6 维恢复 (v1.2.4 6→0 退步 反转)

**跟 §11.3,配合**: "11/16 KPI 达标" 命名 ≠ reality (估数) = "0 实际变化 假动作"

### 3.7 反例 7: v2.1.0 wizard "5-step 引导" 表面 — 实际 user 输入 无 evidence chain

**模式**: v2.1.0 wizard 5-step 引导 (file:line `docs/KALLAX-GLOSSARY.md:521-533`), 自报 "5-step 完成", 缺 per-step user input evidence (跟 dry-run 模式,配合, file:line `docs/KALLAX-GLOSSARY.md:536-547`)
...
**从根源修复** (配合 v2.1.0 dry-run,配合): `--dry-run` 模拟运行, 退出前打印 "Dry-run complete. No files were installed." (file:line `docs/KALLAX-GLOSSARY.md:536-547`)

**跟 §11.3,配合**: "wizard 5-step 完成" 命名 ≠ reality (无 install file:line) = "0 实际变化 假动作"

---

## 4. 七正例 (跟 KALLAX 实际 release 案例,配合)
...
- 原则 3: 跟 §11.3,配合, 命名 (proposal) 跟 reality (实际) 失配 → 诚实标记

**跟 §1.2 诚实修正评估,配合**: 主动 标记 proposal 跟 actual 差异, 不等别人发现

### 4.2 正例 2: v2.0.6 4 工具 multi-tool — install.sh `--target=auto` evidence chain 完整

**模式**: v2.0.6 EPIC-057 4 ticket 完整完成, install.sh `--target=auto` 检测 Claude/opencode/Codex/Gemini 4 工具 (file:line `scripts/install.sh:69-92`, `:139-141`)
...
**配合 v2.0.2 同类症状 从根源修复,配合**: "跨平台" 命名 → v2.0.6 真正 4 工具, naming = reality
...
- 原则 3: 配合 v2.4.0 4 Rule 合并 0 实际变化 不同, worktree 清理 是 **实际 disk 节省**, naming = reality

**跟 §11.5 revert 跟反思 区别,配合**: worktree 清理 保留 (v2.4.1 没 revert), 跟 4 Rule 合并 同类症状 不同

### 4.4 正例 4: v2.4.1 revert 完整完成 — 4 合并 naming 跟 reality 失配 → revert

**模式**: v2.4.1 (commit `7f401f9`) revert v2.4.0 4 Rule 合并, 22 Rule 配合 v2.3.0 一致 (file:line `confluence/decisions/PHASE-013-REFLECTION-2026-06-18.md:284`, `confluence/decisions/PHASE-014-REVIEW-2026-06-18.md:168`)
...
**跟 §11.5 revert 跟反思 区别,配合**: revert 技术 行动 + 反思 战略 行动 完整完成, 跟"诚实修正评估",配合

### 4.5 正例 5: 26 .md wrappers 完整完成 — Claude Code 2.1+ 实测 → 从根源修复 BE-15
...
- 原则 3: 决策者 反馈 完整完成 (file:line `confluence/decisions/ACCUMULATED-LESSONS-2026-06-17.md:698` "决策者 Unknown command 反馈" → v2.1.1 .md wrappers)

**跟 §11.3 0 实际变化 假动作 从根源修复,配合**: v2.0.9-v2.0.11 0 实际变化 (改 .sh 表面) → v2.1.1 从根源修复 (加 .md wrappers)
...
**配合 v2.0.3 11 KPI falsification 从根源修复,配合**: 11 KPI 估数 → v2.0.4 6 维度 raw output 强制

### 4.7 正例 7: v2.3.0 pre-commit ALLOWED_PATTERNS `^jira/` — 1 line diff 完整完成 BE-14
...
- 原则 3: 1 line diff evidence + 0 `--no-verify` 完整完成

**跟 BE-14 从根源修复,配合**: "Todo 1-5 --no-verify workaround 反复" → v2.3.0 `^jira/` 从根源修复

---

## 5. 验证方法 (跟 Master 6 维 L6 诚实,配合)

### 5.1 Master 6 维度验证 (file:line `docs/KALLAX-GLOSSARY.md:625-637`)

| L# | 验证内容 | 命令 | 跟 Fact-Forcing,配合 |
|---|---|---|---|
...
### 5.3 完整完成验证 checklist (跟 §1.5 完整完成,配合)

> **配合 v2.7.4 D5 + /kallax-panel 2026-06-25 9 专家,0 隐藏**: 6 项 checklist 全部 跨 release 留待 落地 实施, 跟"诚实修正评估" 战略,0 形式通过实质失败 校验 100%, 配合 EPIC-059-D Fact-Forcing,配合 100% 落地.

- [ ] evidence chain 3 件套 (file:line / 命令输出 / 代码位置)
- [ ] 跨 release 验证 (不只 落地 commit)
- [ ] raw test output (不接受 "should work")
- [ ] 同类症状 检测 (命名 跟 reality 是否一致)
- [ ] 0 实际变化 检测 (0 增命令 + 净价值 持平 + 0 跨 release 验证 = 0 实际变化)
- [ ] §1.2 诚实修正评估,配合 (主动 标记 proposal 跟 actual 差异)
...
**KALLAX 实证** (跟 BE-15,配合): v2.0.9-v2.0.11 改 .sh 表面 → 决策者反馈仍存 → 撤销 接受自报, 改问 Claude Code 2.1+ registry 实测 → v2.1.1 .md wrappers
...
**KALLAX 实证** (配合 v2.0.5 honest mark,配合): proposal -3 → 实际 -2 诚实标记 (file:line `confluence/decisions/ACCUMULATED-LESSONS-2026-06-17.md:288-289`)
...
**KALLAX 实证** (配合 v2.4.1 revert,配合): v2.4.0 4 合并 净价值 +4% (无 跨 release 验证) → v2.4.1 revert (file:line `confluence/decisions/PHASE-013-REFLECTION-2026-06-18.md:284`)

---

## 7. 跟 KALLAX-GLOSSARY §11.3 "0 实际变化 假动作",配合

### 7.1 §11.3 定义回顾 (file:line `docs/KALLAX-GLOSSARY.md:764-778`)

**§11.3**: "'0 增命令 跟 净价值 持平' 是 0 实际变化, 跟'同类症状',配合, 需 诚实修正评估 — v2.4.0 4 合并 配合 v2.3.0 持平 0 增命令 0 重写主逻辑, '制造 0 实际改变 假动作' 同类症状"
...
**怎么用** (跟 §11.3,配合):
- 检测到 "0 增命令 + 0 重写 + 净价值 持平" → 触发 §11.3 同类症状
- §11.3 触发 → 走 Fact-Forcing 3 原则 验证 evidence chain
- evidence 缺 → 走 revert / 走 fix, 不 模糊 处理

### 7.3 完整完成验证 (跟 §1.5 完整完成,配合)

- ✅ **§12.1 章节**: KALLAX-GLOSSARY.md 落地 (1/1)
- ✅ **fact-forcing.md**: docs/process/ 落地 (1/1)
- ✅ **fact-forcing-examples.md**: confluence/decisions/ 落地 (1/1)
- ✅ **KPI 100% 落地**: 跟 Rule 9 X/Y 格式,配合 (配合 EPIC-059-D ticket AC #9,配合)

---

## 8. 跟 "诚实修正评估" + "同类症状" + "翻篇&精进" 战略,配合

### 8.1 跟"诚实修正评估",配合 (跟 §1.2,配合)

- **Fact-Forcing** = §1.2 诚实修正评估 的 **工具化**
- v2.0.5 honest mark 模式 = 主动 标记 proposal 跟 actual 差异, 不等别人发现
- v2.4.1 revert 模式 = 主动 反思 自己 v2.4.0 决策, 跟"诚实修正评估",配合

### 8.2 跟"同类症状",配合 (跟 §1.1,配合)

- **Fact-Forcing** 是 从根源修复 同类症状 (命名 ≠ reality) 的 **手段**
- BE-15 "Unknown command" 同类症状 → v2.1.1 .md wrappers 从根源修复
- v2.4.0 4 合并 同类症状 → v2.4.1 revert 从根源修复

### 8.3 跟"翻篇&精进",配合 (跟 §2.3,配合)

- **Fact-Forcing** 是 从根源修复 "0 增命令 跟 净价值 持平" 的 **手段**
- 0 增命令 不等于 0 实际变化 (跟 §11.3,配合)
- Fact-Forcing 检测 0 实际变化 → 走 revert / 走 fix, 不 接受 模糊 命名

---

**跟 eket MASTER-RULES.md §2,配合, 借鉴方法论而非直接复制代码, 跟 KALLAX-GLOSSARY §12,配合, 跟 Master 6 维 L6 诚实,配合, 配合 v2.0.5/v2.0.6/v2.4.1 红线 revert 文档化,配合, 配合 v2.4.0 4 合并 假动作,配合, 跟"同类症状" + "诚实修正评估" + "翻篇&精进" 战略,配合, 跟 KALLAX-GLOSSARY §11.3 0 实际变化 假动作,配合, 跟 KALLAX-GLOSSARY §1.1 §1.2 §1.5 §9.3 §11.5,配合, 跟 Rule 9 X/Y + Rule 11 Master 6 维 + Rule 18 KPI Falsification 黑名单,配合, 0 增 Rule, 0 重写, EPIC-059-D 落地**