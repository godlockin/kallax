...
- **v3.5.0 ioredis Pub/Sub** (跟 eket 分布式队列 对照验证):
  - `node/src/core/redis-pubsub.ts` (新增 2026-06, EPIC-060-A Phase 1)
  - `createRedisPubSubBus` + `createInMemoryPubSubBus` (1 interface + 2 implementations, 跟 Rule 5 DRY,配合)
  - **v3.5.0 hotfix (B 组 S-003)**: logger.error / logger.warn 全 redact password (跟 redact-secret.ts 配合)

### §11 武器 1 master-election.ts (增量)

- **v3.5.0 redisPool fd leak 从根源修复** (B 组 S-005):
  - overwrite 旧 connection 前先 quit
  - registerCleanupHandler Node.js exit 时 close 全部 pool

### §12 武器 6 Web Dashboard (增量)

- 配合 v3.5.0 0 增量 (配合 v3.0.0 + v3.1.0 一致)

### §13 实战 (增量)

- **v3.5.0 graceful-exit.sh** (跟 eket Level 4 优雅退出 1:1):
  - 加 `--dry-run` / `--actual` flag (B 组 S-001 从根源修复)
  - 加 SIGTERM/SIGINT trap handler (B 组 S-002 从根源修复)
  - pid_file (.kallax/pid/*.pid) 优先 + pgrep -o 兜底 (B 组 S-002 从根源修复)
...
## v3.5.0 CLAUDE.md 应增量 (跟 U-004,配合)

```
## 13. caveman mode (配合 v3.2.0 rtk + caveman 整合,配合, 75% token 节省)
   - /caveman 切换简化输出
   - /caveman + /kallax-* 兼容
```

## 跟 B 组 U-001 从根源修复,配合

Iter 2 锁定 docs/ARCHITECTURE.md / docs/CHEATSHEET.md / CLAUDE.md, 本 DELTA 文档 1:1 列 增量, 待决策者拍板 v3.6.0 整合. 跟 V310-B U-002 / U-005 / U-007 配合: 留待决策者拍板, 不擅自改 locked docs.
